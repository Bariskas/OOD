...
{
public:
	CChocolate(IBeveragePtr && beverage, unsigned slicesCount)
		: CCondimentDecorator(move(beverage))
	{
		m_slicesCount = slicesCount > 5 ? 5 : slicesCount;
	}
protected:
	double GetCondimentCost()const override
	{
		return 10 * m_slicesCount;
	}
	std::string GetCondimentDescription()const override
	{
		return "Chocolate x" + std::to_string(m_slicesCount) + " slices";
	}
private:
	unsigned m_slicesCount;
};
...
protected:
	...
	std::string GetCondimentDescription()const override
	{
		return "Liquor from " + getLiquorMaterial();
	}
private:
	std::string getLiquorMaterial()const
	{
		std::string material;
		switch (m_type)
		{
